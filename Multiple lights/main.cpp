...
//    glEnableVertexAttribArray(2);
    ...
    unsigned char *data = SOIL_load_image("../container2.png",
    ...
    unsigned char *data1 = SOIL_load_image("../container2_specular.png",
...
//    cubeShader.setVec3("material.specular", vec3(0.5f, 0.5f, 0.5f));
    ...
    cubeShader.setVec3("dirLight.ambient",  vec3(0.2f, 0.2f, 0.2f));
    cubeShader.setVec3("dirLight.diffuse",  vec3(0.5f, 0.5f, 0.5f)); // 将光照调暗了一些以搭配场景
    cubeShader.setVec3("dirLight.specular", vec3(1.0f, 1.0f, 1.0f));
    ...
    for (int i = 0; i < 4; i ++) {
        ...
        cubeShader.setVec3("pointLights[i].ambient",  vec3(0.2f, 0.2f, 0.2f));
        cubeShader.setVec3("pointLights[i].diffuse",  vec3(0.5f, 0.5f, 0.5f)); // 将光照调暗了一些以搭配场景
        cubeShader.setVec3("pointLights[i].specular", vec3(1.0f, 1.0f, 1.0f));
        cubeShader.setVec3("pointLights[i].position", pointLightPositions[i]);


        cubeShader.setFloat("pointLights[i].constant",  1.0f);
        cubeShader.setFloat("pointLights[i].linear",    0.09f);
        cubeShader.setFloat("pointLights[i].quadratic", 0.032f);
    }
...
//        model1 = glm::translate(model1,  glm::vec3 (1.0f, 1.2f, 2.0f));
        ...
        for (unsigned int i = 0; i < 10; i++) {
            glm::mat4 model11 = mat4(1.0f);
            model11 = glm::translate(model11, cubePositions[i]);
            float angle = 20.0f * i;
            model11 = glm::rotate(model11, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
            cubeShader.setMat4("model", model11);
        ...
        for (int i = 0; i < 4; i ++) {
            vec3 pos = pointLightPositions[i];
